...
# 窗口扰动技术 - 防止截图/录屏
def disturb_window(root_ref):
    while not global_exit:
        if not hidden and root_ref.winfo_exists():
            try:
                # 随机改变窗口位置
...
def check_password():
    ...
    try:
        ...
        return False  # 防止解锁对话框错误

# 修改窗口样式属性
def modify_window_style(root_ref):
    if sys.platform == "win32":
        try:
            hwnd = win32gui.FindWindow(None, "安全计算器")
            # 移除窗口边框
...
def protect_process():
    def block_task_manager():
        while not global_exit:
            try:
                # 尝试查找任务管理器窗口
                hwnd = win32gui.FindWindow(None, "Windows 任务管理器")
                if hwnd:
                    # 隐藏任务管理器
...
def create_calculator():
    global root, history, hidden, global_exit
    ...
    # 修改窗口样式
    if sys.platform == "win32":
        root.after(100, lambda: modify_window_style(root))
    
    # 启动窗口扰动线程
    disturb_thread = threading.Thread(target=lambda: disturb_window(root), daemon=True)
    ...
    def on_esc_press(event):
        ...
        if event.keysym == 'Escape':
            if messagebox.askyesno("退出", "确定要退出所有计算器吗？"):
                global_exit = True
                root.quit()
                root.destroy()
...
if __name__ == "__main__":
    ...
    processes = []
    for _ in range(3):
        p = multiprocessing.Process(target=calculator_process)
        p.daemon = True
        p.start()
        processes.append(p)
        time.sleep(0.5)  # 稍微延迟，避免窗口完全重叠
    ...
    # 设置超时保护防止卡死
    hotkey_thread.join(timeout=1)  # 只等待1秒
    
    # 等待所有进程完成
    for p in processes:
        try:
            p.join(timeout=1)  # 设置超时防止无限等待
        except:
            pass
